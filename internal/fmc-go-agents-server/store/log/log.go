...
var (
	logMaps = make(map[string]*fzlog.UzLog)
	sysLog  *fzlog.UzLog // 系统日志
	msgLog  *fzlog.UzLog // 消息日志
...
// 返回: 无
func NewLog(config *fzlog.Option) {
	var c *fzlog.Option
	once.Do(func() {
		if config == nil {
			c = fzlog.NewDefaultOption()
		} else {
			c = config
		}
		err := c.Verify()
		...
		c.Name = "sysLog"
		sysLog = fzlog.NewUzLog(c)
		c.Name = "msgLog"
		msgLog = fzlog.NewUzLog(c)
	})
	if sysLog == nil {
		panic("sysLog is nil")
	}
