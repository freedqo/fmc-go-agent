...
func NewMessageForComLinkToDataSourceStateChanged(clientID string, oldState, newState LinkToDataSourceState, linkDataSources DataSourceID, desc string) msgm.TAiAgentMessage {
	dss := []DataSourceID{linkDataSources}
	return NewMessageForComLinkToDataSourceStateChanged2(clientID, oldState, newState, dss, desc)
}

// 创建用于推送一个到数据源的连接状态发生变化消息（用于服务端）
func NewMessageForComLinkToDataSourceStateChanged2(clientID string, oldState LinkToDataSourceState, newState LinkToDataSourceState, linkDataSources []DataSourceID, desc string) msgm.TAiAgentMessage {
	operateData := NewPushComLinkToDataSourceStateChangedOperateData(oldState, newState, linkDataSources, desc)
	return fmsg.NewMessageForSend(clientID, string("MOT_COM_LinkToDataSourceStateChanged"), fmt.Sprintf("%T", operateData), operateData)
}
