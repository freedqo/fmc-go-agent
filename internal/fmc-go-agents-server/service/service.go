...
func New(ctx context.Context, opt *config.Config) If {
	...
	// 实例mcp服务
	s.mcp = mcpsrv.New(ctx, opt, s.dal, s.msg.Publish, s.msg.WaitingResMsg)
	...
	// 实例eino服务,并注入会话管理,注入消息发送器
	s.eino = einosrv.New(ctx, opt, s.dal, s.mcp, s.session, s.msg.Publish)
