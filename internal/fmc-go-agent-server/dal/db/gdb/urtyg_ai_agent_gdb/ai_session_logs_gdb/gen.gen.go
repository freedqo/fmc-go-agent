...
func newGenIF(gdb *gorm.DB) ai_session_logs_if.GenIf {
	return &GenIF{
		db: gdb,
	}
}
...
// Add 批量添加数据
func (g *GenIF) Add(data ...*model.Ai_session_logs) error {
...
// Upt 批量更新数据
func (g *GenIF) Upt(data ...*model.Ai_session_logs) error {
...
// Save 批量保存数据
func (g *GenIF) Save(data ...*model.Ai_session_logs) error {
	return g.db.Save(data).Error
}

// Del 批量删除数据
func (g *GenIF) Del(data ...*model.Ai_session_logs) error {
	return  g.db.Delete(data).Error
}

// First 查询第一条数据
func (g *GenIF) First(query *model.Ai_session_logs) (data *model.Ai_session_logs, err error) {
	data = &model.Ai_session_logs{}
	db := g.db
...
// Find 根据查询条件查询数据
func (g *GenIF) Find(query *model.Ai_session_logs_QueryReq) (data []*model.Ai_session_logs, total int64, err error) {
	db := g.db
	...
	if query != nil {
		...
		// 最后时间
		addQueryCondition("last_at", query.LastAt, query.IsLike)
		// 提示词模板id
		addQueryCondition("prompt_id", query.PromptID, query.IsLike)
		// 用户id
	...
	err = countDB.Model(&model.Ai_session_logs{}).Count(&total).Error
	if err != nil {
		return nil, 0, err
	}
...
var _ ai_session_logs_if.GenIf = &GenIF{}
