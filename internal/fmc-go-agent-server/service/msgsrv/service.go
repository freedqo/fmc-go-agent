...
import (
	...
	"github.com/freedqo/fmc-go-agent/internal/fmc-go-agent-server/service/msgsrv/handler"
	"github.com/freedqo/fmc-go-agent/internal/fmc-go-agent-server/service/msgsrv/hub"
...
func New(cfg *config.Config) If {
	s := Service{
		...
		mu:              sync.Mutex{},
		hub:             hub.New(cfg),
		inMsgChan:       make(chan *umsg.UMsg, InMsgCacheSize),  //需要处理的消息队列
		outMsgChan:      make(chan *umsg.UMsg, OutMsgCacheSize), //需要推送的消息队列
		handlerInMsgFun: make(map[*umsg.HandlerInMsgFun]*umsg.HandlerInMsgFun),
	}
	// 注册消息处理服务,消息输入方法
...
type Service struct {
	...
	inMsgChan  chan *umsg.UMsg //需要处理的消息队列
	outMsgChan chan *umsg.UMsg //需要推送的消息队列

	handlerInMsgFun   map[*umsg.HandlerInMsgFun]*umsg.HandlerInMsgFun //待处理消息消息的处理器
	vHandlerInMsgFunc umsg.HandlerInMsgFun                            //固化的待处理消息的处理器

	vPublish umsg.RecEventFunc
}

// GetMsgHubIf 获取消息hub接口
// 入参： name string 消息hub名称
// 返回： 消息hub接口实例
func (s *Service) GetMsgHubIf(name string) umsg.MessageHubIf {
...
func (s *Service) WaitingResMsg(operateID string, waitTime time.Duration) (*umsg.UMsg, error) {
	if waitTime <= 0 {
	...
	// 创建一个消息通道，用于接收消息
	msgChan := make(chan *umsg.UMsg, 1)
	// 定义一个消息处理函数
	var vhandler umsg.HandlerInMsgFun
	// 是否已经接收到了消息
	isRec := false
	// 定义消息处理函数
	vhandler = func(push umsg.PublishFunc, msg *umsg.UMsg) {
		// 如果还没有接收到消息,且消息的OperateID与传入的operateID相等,且消息是应答消息
		if !isRec && msg.Msg.OperateID == operateID && msg.Msg.MessageBase.IsReplyOperate {
			// 将消息发送到消息通道
			msgChan <- msg
			// 设置已经接收到消息
			isRec = true
...
// 入参： msg *umsg.UMsg 消息
// 返回： 无
func (s *Service) push(msg *umsg.UMsg) {
...
// Subscribe 订阅
// 入参： fun *umsg.HandlerInMsgFun 消息处理器
// 返回： 无
func (s *Service) Subscribe(fun *umsg.HandlerInMsgFun) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// 入参： fun *umsg.HandlerInMsgFun 消息处理器
// 返回： 无
func (s *Service) UnSubscribe(fun *umsg.HandlerInMsgFun) {
	s.mu.Lock()
	defer s.mu.Unlock()
