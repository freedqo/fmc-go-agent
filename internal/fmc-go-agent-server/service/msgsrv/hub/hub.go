...
import (
	...
	"github.com/freedqo/fmc-go-agent/internal/fmc-go-agent-server/service/msgsrv/hub/mainsrvws"
	"github.com/freedqo/fmc-go-agent/internal/fmc-go-agent-server/service/msgsrv/hub/mqtt"
...
func New(cfg *config.Config) If {
	hub := Hub{
		bus: make(map[string]umsg.MessageHubIf, 0),
		wss: make(map[string]umsg.MessageHubIf, 0),
		mqt: make(map[string]umsg.MessageHubIf, 0),
	}
	// 注入wss消息实例
...
type Hub struct {
	...
	bus    map[string]umsg.MessageHubIf
	wss    map[string]umsg.MessageHubIf
	mqt    map[string]umsg.MessageHubIf
}

// GetMsgHubIf 获取wss消息集线器
// 入参：name string 消息集线器名称
// 返回：umsg.MessageHubIf 消息集线器接口
func (h *Hub) GetMsgHubIf(name string) umsg.MessageHubIf {
	v, ok := h.bus[name]
	if ok {
		return v
	}
...
// 入参：fun *umsg.RecEventFunc 消息接收事件函数
// 返回：无
func (h *Hub) SubscribeRecEvent(fun *umsg.RecEventFunc) {
	for _, v := range h.bus {
...
// 入参：fun *umsg.RecEventFunc 消息接收事件函数
// 返回：无
func (h *Hub) UnSubscribeRecEvent(fun *umsg.RecEventFunc) {
	for _, v := range h.bus {
...
// 入参：msg *umsg.UMsg 消息
// 返回：无
func (h *Hub) Publish(msg *umsg.UMsg) {
