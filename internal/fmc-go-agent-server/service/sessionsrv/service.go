...
import (
	...
	"github.com/freedqo/fmc-go-agent/internal/fmc-go-agent-server/service/sessionsrv/dbconversation"
...
func New(ctx context.Context, opt *config.Config, dal dal.If) If {
	...
	s := &Service{
		...
		opt:  opt,
		dal:  dal,
		uJwt: ujwt.New("fmc-go-ai-agent", "your jwt key"),
		mu:   sync.Mutex{},
	}
	return s
}
...
func (s *Service) QuerySessionChatLogsByUser(ctx context.Context, req sessionm.QuerySessionChatLogsByUserReq) (*sessionm.QuerySessionChatLogsByUserResp, error) {
	...
	if req.PromptType != nil {
		_, ok := iconsts.PromptTypeDist[iconsts.PromptType(*req.PromptType)]
		if !ok {
			return nil, fmt.Errorf("promptType is invalid")
		}
	}
	res := &sessionm.QuerySessionChatLogsByUserResp{
		SessionId:  "",
		PromptType: "",
		ChatLogs:   make([]*model.Ai_chat_logs, 0),
	}
	// 先查用户所有的会话记录
	// 查提示词模板id,根据用户id和type
	...
	if first == nil {
		return nil, errors.New(fmt.Sprintf("用户未配置,类型为%s的提示词", iconsts.PromptTypeDist[iconsts.PromptType(*req.PromptType)]))
	}
	...
	if len(sessionLogs) == 0 {
		...
		if req.PromptType != nil {
			...
			res.PromptType = string(iconsts.PromptID_IntelligentAssistant) //提供默认的AI智能助手提示词
		}
		sessionId := fmt.Sprintf("%d", utils.GetIntID())
		session, err := s.uJwt.GetSession(req.UserId, sessionId)
		if err != nil {
