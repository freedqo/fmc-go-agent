...
// 定义一个函数，用于构建一个可运行的节点
func (u *FAiAgent) buildRunnable(ctx context.Context, ctp prompt.ChatTemplate, lba *compose.Lambda, rtr retriever.Retriever) (g *compose.Graph[*UserMessage, *schema.Message], r compose.Runnable[*UserMessage, *schema.Message], err error) {
	// 定义一些常量
	const (
	...
	// 编译图
	r, err = g.Compile(ctx, compose.WithGraphName("FAiAgent"), compose.WithNodeTriggerMode(compose.AllPredecessor))
	if err != nil {
		return nil, nil, err
	}
	return g, r, err
