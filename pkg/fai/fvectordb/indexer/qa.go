...
func NewQa(cfg *openai.ChatModelConfig) *Indexer {
...
func (i *Indexer) Qa(ctx context.Context, docs []*schema.Document) (output []*schema.Document, err error) {
	...
	for _, doc := range docs {
		...
		go func(doc *schema.Document) {
			defer wg.Done()
			qaContent, e := getQAContent(ctx, doc, knowledgeName, i.cfg)
			if e != nil {
				return
			}
...
func getQAContent(ctx context.Context, doc *schema.Document, knowledgeName string, cfg *openai.ChatModelConfig) (qaContent string, err error) {
	// 已经有数据了就不要再生成了
	if s, ok := doc.MetaData[common.FieldQAContent].(string); ok && len(s) > 0 {
		return s, nil
	}

	cm, err := common.GetQAModel(ctx, cfg)
	if err != nil {
		return
	}
