...
func (m *FChatModel) BindTools(tools []*schema.ToolInfo) error {
	return m.einoCm.BindTools(tools)
}

func (m *FChatModel) V1() *openai.Client {
	return m.openai
}

func (m *FChatModel) Generate(ctx context.Context, input []*schema.Message, opts ...model.Option) (*schema.Message, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if opts != nil {
...
func (m *FChatModel) Stream(ctx context.Context, input []*schema.Message, opts ...model.Option) (*schema.StreamReader[*schema.Message], error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.einoCm.Stream(ctx, input, opts...)
}

func (m *FChatModel) WithTools(tools []*schema.ToolInfo) (model.ToolCallingChatModel, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *FChatModel) SetProvider(provider ModelProvider) error {
	// 检查提供的模型提供者是否在支持列表中
	...
	case Qwen:
		{
			c, err := qwen.NewChatModel(m.ctx, &qwen.ChatModelConfig{
				APIKey:  m.opt.APIKey,
				Timeout: time.Duration(m.opt.Timeout) * time.Second,
				BaseURL: m.opt.BaseURL,
				Model:   m.opt.Model,
