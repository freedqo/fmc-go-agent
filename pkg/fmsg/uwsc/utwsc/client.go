...
func NewClient() *Client {
	...
	client := &Client{Cfg: &cfg,
		...
		conn:            nil,
		ReceiveMsg:      make(chan fmsg.InMessage, receiveMsgSize),
...
// msg.SessionId - 为本地客户端GUID。一般情况下，要和StartService（）方法中的clientID参数保持一致，运行过程不会改变
func (c *Client) SendMessage(msg msgm.TAiAgentMessage) (err error) {
	...
	if c.isTransparentTransport {
		// 先将该消息进行业务处理。（可选）
		if msg.OperateDataType != fmsg.TransparentTransport_package_message_OperateDataType || msg.Data == nil {
...
func (c *Client) setCurConnected(isConnected bool) {
	...
	if c.cnnState != isConnected {
		...
		c.ClientConnState <- fmsg.NewClientConnectState(c.clientGuid, isConnected, c.GetRemoteAddrString())
	}
}
...
func (c *Client) dialAndServiceOnce(ctx context.Context) (done <-chan struct{}, err error) {
	...
	go func() {
		...
		//采用Message格式消息，要发送握手
		if !c.isTransparentTransport {
			msg := fmsg.NewMessageForSend(c.getClientGuid(), "", "", nil)
			omsg, err2 := fmsg.NewOutMessage(msg)
			if err2 == nil {
		...
		go func() {
			...
			for {
				...
				if c.isTransparentTransport {
					...
					packageMsg := fmsg.NewMessageForPackage(message)
					packageMsgData, err := json.Marshal(packageMsg)
					if err != nil {
						c.log.Debugf("%v ←→ %v 解码错误：%v", c.conn.LocalAddr(), c.conn.RemoteAddr(), err)
						continue
					}
					var msgBase fmsg.MessageBase = packageMsg.MessageBase //复制
					inMsg, err := fmsg.NewInMessage(msgBase, packageMsgData)
					if err == nil {
					...
					//对外输出接收的消息
					inMsg, err := fmsg.NewInMessage(msgBase, message)
					if err == nil {
						if len(c.ReceiveMsg) == cap(c.ReceiveMsg) {
							<-c.ReceiveMsg //去掉旧的，保留最新的,以防缓存满后，造成阻塞
						}
