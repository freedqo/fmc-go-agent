...
import (
	...
	utwss2 "github.com/freedqo/fmc-go-agents/pkg/fmsg/uwss/utwss"
...
func New(name string, cfg *Option, log *zap.SugaredLogger) If {
	if log == nil {
		panic("ut ws server log is nil")
	}
	if cfg == nil {
		panic("ut ws server cfg is nil")
	}
	if name == "" {
		panic("ut ws server name is nil")
	}
	return &UWss{
		mu:            sync.Mutex{},
		Name:          name,
...
func (u *UWss) GetConnectState() fmsg.ClientConnectState {
	return *u.connState
}

func (u *UWss) Publish(msg *fmsg.UMsg) {
	u.outUMsgChan <- msg
}

func (u *UWss) SubscribeRecEvent(fun *fmsg.RecEventFunc) {
	u.mu.Lock()
	defer u.mu.Unlock()
	u.recUMsgFunMap[fun] = fun
}

func (u *UWss) UnSubscribeRecEvent(fun *fmsg.RecEventFunc) {
	u.mu.Lock()
	defer u.mu.Unlock()
			...
			case msg, ok := <-u.wss.ReceiveMsg:
				{
					if ok {
						if u.recUMsgFunMap != nil && len(u.recUMsgFunMap) > 0 {
							utmsg := fmsg.NewUMsg(&msgm.TAiAgentMessage{
