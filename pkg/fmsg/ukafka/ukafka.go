...
type UKafka struct {
	...
	recFuncs             []*fmsg.RecEventFunc
	connectState         fmsg.ClientConnectState
	mu                   sync.RWMutex
	wg                   sync.WaitGroup
	receivingQueue       chan *sarama.ConsumerMessage
	sendingQueue         chan *fmsg.UMsg
...
func New(name string, opt *Options, log *zap.SugaredLogger) If {
	...
	k := &UKafka{
		...
		recFuncs:             make([]*fmsg.RecEventFunc, 0),
		receivingQueue:       make(chan *sarama.ConsumerMessage, 10*1024),
		sendingQueue:         make(chan *fmsg.UMsg, 10*1024),
		reconnectAttempts:    0,
...
// 构建Kafka生产者消息
func (k *UKafka) buildKafkaMessage(msg *fmsg.UMsg) (*sarama.ProducerMessage, error) {
	// 将消息转换为JSON格式
	dataJson, err := json.Marshal(msg.Msg.Data)
	if err != nil {
		return nil, err
	}
	...
	kTopic, ok := k.publishTopicMap[msg.Msg.Topic]
	if !ok {
		return nil, fmt.Errorf("未找到对应的Topic,topic:%s", msg.Msg.Topic)
	}
...
func (k *UKafka) handleSendMsg(msg *fmsg.UMsg) {
	// 检查连接状态
	k.mu.RLock()
	...
	if !connected {
		k.log.Warnf("Kafka连接断开，消息将被丢弃: %v", msg.Msg.Topic)
		return
	}
...
// Publish 推送消息到Kafka
func (k *UKafka) Publish(msg *fmsg.UMsg) {
	defer func() {
		if err := recover(); err != nil {
			// 生成一个唯一的错误ID，用于后续的错误跟踪
			PanicId := uuid.New().ID()
...
func (k *UKafka) SubscribeRecEvent(fun *fmsg.RecEventFunc) {
	k.mu.Lock()
	defer k.mu.Unlock()
...
func (k *UKafka) UnSubscribeRecEvent(fun *fmsg.RecEventFunc) {
	k.mu.Lock()
	defer k.mu.Unlock()
...
func (k *UKafka) GetConnectState() fmsg.ClientConnectState {
	k.mu.RLock()
	defer k.mu.RUnlock()
