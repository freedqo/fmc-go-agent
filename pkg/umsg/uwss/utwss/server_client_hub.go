...
func newHub(log *zap.SugaredLogger) *hub {
	return &hub{
		...
		SendMsg:            make(chan umsg.OutMessage, 1024),
...
type hub struct {
	...
	ReceiveMsg chan umsg.InMessage  //管道：对外输出接收到的消息
	SendMsg    chan umsg.OutMessage //管道：接收外部请发送到客户端的消息
	...
	go func() {
		...
		for {
			...
			case imsg := <-h.iMsg: //从客户端接收到消息
				...
				// 强制刷新客户端对应的ClientID
				msgBase.ClientID = imsg.Client.Id

				//对外输出接收的消息
				inMsg, err := umsg.NewInMessage(msgBase.MessageBase, msgBase.OperateData)
				if err == nil {
