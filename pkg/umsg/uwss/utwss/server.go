...
func New(log *zap.SugaredLogger, opt *Option) *Server {
	server := &Server{
		hub:             newHub(log),
		ReceiveMsg:      make(chan umsg.InMessage, 1024),
...
func (s *Server) SendMessage(msg umsg.Message) (err error) {
	defer func() {
		if e := recover(); e != nil {
			err = errors.New(fmt.Sprintln(e))
			s.log.Errorf("%v", err) //输出到致命（关键错误）日志跟踪器
	...
	if msg.ClientID != "" {
		_, ok := s.hub.ClientidToClients[msg.ClientID]
		if !ok {
			if msg.OperateID != "" {
				msg.MessageBase.IsReplyOperate = true
				cbMsg, err := umsg.NewInMessage(msg.MessageBase, "操作发送失败,远程Web客户端不在线")
				if err != nil {
					return fmt.Errorf("目标客户端[%s]离线,且构建回调消息异常：%v", msg.ClientID, err)
				}
				s.ReceiveMsg <- cbMsg
			}
			return fmt.Errorf("目标客户端[%s]离线", msg.ClientID)
		}
	}
		...
		for {
			...
			case clientInfo, ok := <-s.hub.UnregisterClientID:
				if ok {
					...
					s.ClientConnState <- umsg.NewClientConnectState(clientInfo.ClientID, false, remoteAddr)
			...
			case clientInfo, ok := <-s.hub.RegisterClientID:
				if ok {
					...
					s.ClientConnState <- umsg.NewClientConnectState(clientInfo.ClientID, true, remoteAddr)
