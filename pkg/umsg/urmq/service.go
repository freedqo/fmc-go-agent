...
import (
	...
	"github.com/freedqo/fmc-go-agent/pkg/rocketmqx"
...
func New(name string, o *Option, topicList []string, log *zap.SugaredLogger) *URocketMq {
	...
	r := &URocketMq{
		...
		recUMsgFunMap: map[*umsg.RecEventFunc]*umsg.RecEventFunc{},
		TopicList:     topicList,
		cfg:           o,
	}
	return r
...
func (u *URocketMq) GetConnectState() umsg.ClientConnectState {
	//TODO implement me
	panic("implement me")
}

func (u *URocketMq) Publish(msg *umsg.UMsg) {
	u.outUMsgChan <- msg
}

func (u *URocketMq) SubscribeRecEvent(fun *umsg.RecEventFunc) {
	u.mu.Lock()
	defer u.mu.Unlock()
	u.recUMsgFunMap[fun] = fun
}

func (u *URocketMq) UnSubscribeRecEvent(fun *umsg.RecEventFunc) {
	u.mu.Lock()
	defer u.mu.Unlock()
	...
	go func() {
		...
		for {
			...
			case msg, ok := <-u.outUMsgChan:
				{
					if ok {
						omsg := rocketmqx.NewMessage(msg.Msg.Operate)
						omsg.AddBody(msg.Msg.OperateData)
						if msg.IsASync {
...
func (u *URocketMq) onReceiveUMsg(msg *umsg.UMsg) {
	u.mu.Lock()
	defer u.mu.Unlock()
