...
import (
	...
	utwsc2 "github.com/freedqo/fmc-go-agent/pkg/umsg/uwsc/utwsc"
...
func (u *UWsc) GetConnectState() umsg.ClientConnectState {
	return *u.connState
}

func (u *UWsc) Publish(msg *umsg.UMsg) {
	u.outUMsgChan <- msg
}

func (u *UWsc) SubscribeRecEvent(fun *umsg.RecEventFunc) {
	u.mu.Lock()
	defer u.mu.Unlock()
	u.recUMsgFunMap[fun] = fun
}

func (u *UWsc) UnSubscribeRecEvent(fun *umsg.RecEventFunc) {
	u.mu.Lock()
	defer u.mu.Unlock()
